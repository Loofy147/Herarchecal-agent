...
from mas.hr_rl.core import get_hierarchical_state_representation
from mas.hr_rl.enhanced_agent import EnhancedDQNAgent
...
if __name__ == "__main__":
    ...
    logging.info("🔥 RED TEAM TESTING FRAMEWORK 🔥")
    logging.info("Testing all 6 critical industry gaps")
        ...
        for method_name in dir(test_class):
            if method_name.startswith('test_'):
                try:
                    ...
                    logging.error(f"❌ TEST FAILED: {method_name} - {e}")
                    failed += 1
                except Exception as e:
                    logging.error(f"⚠️ TEST ERROR: {method_name} - {e}", exc_info=True)
                    failed += 1
