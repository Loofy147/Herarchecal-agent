...
from models import LinearUVFA, NeuralUVFA, encode_relational

class HRLTrainer:
    def __init__(self, env, worker, config=CONFIG):
        self.env = env
        self.config = config
        ...
        self.coarse = config["MANAGER_STATE_QUANTIZATION"]
    ...
    def manager_select(self, key, eps=None):
        if eps is None:
    ...
    def manager_update(self, key, subgoal, ret, alpha=None):
        if alpha is None:
    ...
    def run_episode(self, use_prioritized=False, prio_buffer=None):
        ...
        subgoal = self.manager_select(manager_key)
        manager_ret = 0.0
        subgoal_timer = 0
        ...
        while True:
            ...
            if len(self.replay) >= self.config["ONLINE_BATCH_SIZE"]:
