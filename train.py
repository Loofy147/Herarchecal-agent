...
def run_experiment(seed=0, episodes=CONFIG["EPISODES"], use_prioritized=False, tuned=False, use_nn=False):
    ...
    trainer = HRLTrainer(env, worker, CONFIG)
    ...
    for ep in range(episodes):
        ...
        if use_prioritized and prio is not None and len(prio)>=CONFIG["PRIO_BATCH_SIZE"]:
            samples, idxs, weights = prio.sample(CONFIG["PRIO_BATCH_SIZE"])
            td_list=[]
            for tr in samples:
                ...
                if isinstance(trainer.worker, NeuralUVFA):
                    with torch.no_grad():
                        target = tr.r + (0.0 if tr.done else trainer.gamma * torch.max(q2))
                    td = trainer.worker.update(sfeat, tr.a, target)
                else:
